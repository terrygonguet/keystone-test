...
(function () {
	...
	matterjs.src = 'https://cdnjs.cloudflare.com/ajax/libs/matter-js/0.12.0/matter.min.js';
	...
	function setup () {
		...
		let game = {
			...
			paddle: Matter.Bodies.rectangle(innerWidth / 2, 50, $('#navigationList').width(), $('#navigationList').height(), {
				isStatic: true, label: 'Paddle',
			}),
			pWidth: $('#navigationList').width(),
			pHeight: $('#navigationList').height(),
			ball: Matter.Bodies.circle(innerWidth / 2, innerHeight / 3, 15, { label: 'Ball' }),
			ballEl: ball,
			ballSpeed: 9,
		};
		...
		let rect = Matter.Bodies.rectangle(-5, innerHeight / 2, 10, innerHeight, { isStatic: true, label: 'Wall Left' }); // left wall
		Matter.World.addBody(game.world, rect);
		rect = Matter.Bodies.rectangle(innerWidth + 5, innerHeight / 2, 10, innerHeight, { isStatic: true, label: 'Wall Right' }); // right wall
		Matter.World.addBody(game.world, rect);
		rect = Matter.Bodies.rectangle(innerWidth / 2, -5, innerWidth, 10, { isStatic: true, label: 'Wall Top' }); // top wall
		Matter.World.addBody(game.world, rect);
		rect = Matter.Bodies.rectangle(innerWidth / 2, innerHeight + 5, innerWidth, 10, { isStatic: true, label: 'Wall Bottom' }); // bottom wall
		Matter.World.addBody(game.world, rect);
		...
		for (let i = 0; i < 20; i++) {
			...
			let brick = Matter.Bodies.rectangle(
				(0.10 + (x * 0.01) + (x + 0.5) * brickW) * innerWidth,
				(0.9 - (y * 0.01) - (y + 0.5) * brickH) * innerHeight,
				brickW * innerWidth, brickH * innerHeight,
		...
		Matter.Events.on(game.engine, 'collisionStart', function (e) {
			e.pairs.forEach(p => {
				let brick = (p.bodyA.label === 'Brick' ? p.bodyA : (p.bodyB.label === 'Brick' ? p.bodyB : null));
				let paddle = (p.bodyA.label === 'Paddle' ? p.bodyA : (p.bodyB.label === 'Paddle' ? p.bodyB : null));
				let ball = (p.bodyA.label === 'Ball' ? p.bodyA : (p.bodyB.label === 'Ball' ? p.bodyB : null));
				let wallTop = (p.bodyA.label === 'Wall Top' ? p.bodyA : (p.bodyB.label === 'Wall Top' ? p.bodyB : null));
				if (brick) {
					brick.lives--;
					if (brick.lives > 0) {
				...
				if (wallTop) {
					Matter.Body.setPosition(ball, { x: innerWidth / 2, y: innerHeight / 3 });
